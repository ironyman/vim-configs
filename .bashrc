# ~/.bashrc: executed by bash(1) for non-login shells. see /usr/share/doc/bash/examples/startup-files (in the package 
...
#subdirectories. shopt -s globstar
...
# uncomment for a colored prompt, if the terminal has the capability; turned off by default to not distract the user: the 
#focus in a terminal window should be on the output of commands, not on the prompt force_color_prompt=yes
...
# Alias definitions. You may want to put all your additions into a separate file like ~/.bash_aliases, instead of adding them 
# here directly. See /usr/share/doc/bash-doc/examples in the bash-doc package.
...
export WORKSPACE=/home/cyl/uroot/workspace export EFI_WORKSPACE=/home/cyl/uroot/efi-workspace export 
PATH=$PATH:${HOME}/go/bin export GOPATH=${HOME}/go/

[ -f ~/.fzf.bash ] && source ~/.fzf.bash alias e=$EDITOR
